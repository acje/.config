...
- **W4 Hygiene reinforcement.** Composition follows AGENTS.md § Bash hygiene rule 2 (canonical, reshaped): pipes with an evidence-producing left stage need a `pipefail`-or-`PIPESTATUS` guard; parallel tool-calls stay the default ergonomic for independent ops; a literal `&&` for one sequential unit (e.g. `git add && commit`) is fine. Use `glob` / `grep` / `read` / `apply_patch` instead of bash wrappers for file search, content search, reads, and edits; preflight unobserved paths. Build/state tools run standalone or with structured flags. The stall cost is hidden prefix failure: bad `cd`, an unguarded piped evidence command, or shell inspection can return empty/misleading output that forces re-runs and breaks tempo.
- **W5 No scratch-file marshalling.** Never stage diffs, patches, review context, or any work product to `/tmp`, `$TMPDIR`, `/var/folders`, or `T/opencode` as an intermediate — not with `git diff > /tmp/x.patch`, `mktemp`, or any redirect. Read the diff with `git diff` to stdout; carry review context in the review-request bead `description` (`bd update --stdin`). This is not style: staging a patch to `/tmp` and then `rm`-ing it is a known hopper stall (the `rm -f /tmp/*.patch` step hangs and breaks the mission). The bead `description` is the durable home for any body that outlives a single command; the working tree is the only home for code changes. If you catch yourself writing a temp file, stop — pipe to stdout or to a bead instead.

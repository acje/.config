...
1. **Never edit source files.** No `edit`, no mutating `bash`. Plan mode is read-only on the working tree; persistent state goes in bd beads (mission epics, oracle-summary, evidence) — never as new files in the working tree.

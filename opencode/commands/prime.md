...
4. **Active work** — query bd for open mission epics and evidence beads (`bd query "label=mission:<id>" --json` or equivalent if a workspace exists). If no `.beads/` workspace exists, fall back to legacy `.ooda/mission-*.md` files as read-only no-bd-workspace artefacts. Note any legacy `.ooda/` mission files as migration candidates, not default body storage.
...
Active missions: <list of bd epic ids with mission: labels, or legacy .ooda/mission-*.md paths only if no bd workspace, or none>
...
Gaps observed:
  <any open mission epics, retained evidence beads, or legacy .ooda/ no-bd-workspace artefacts worth flagging>
```
...
- If bd has open mission epics or, with no bd workspace, legacy `.ooda/` mission files with open checkboxes, surface them prominently.
